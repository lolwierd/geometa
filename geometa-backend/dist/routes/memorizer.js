...
import { getCountriesByContinent } from '../lib/continents.js';
...
router.get('/', async (req, res) => {
    try {
        ...
        const conditions = ["mp.due_date <= strftime('%s','now')"];
        const params = [];
        // Add country filter
...
router.post('/:id', async (req, res) => {
    ...
    `);
        const current = currentStmt.get(locationId);
        if (!current) {
            return res.status(404).json({ error: "Progress not found for this location" });
        }
        // Calculate next review based on quality
        const nextReview = calculateNextReview(quality, current.repetitions, current.ease_factor, current.interval, current.state, current.lapses);
        // Update progress
      ...
      RETURNING *
    `);
        const updatedProgress = updateStmt.get(nextReview.repetitions, nextReview.easeFactor, nextReview.interval, Math.floor(Date.now() / 1000) + (nextReview.interval * 24 * 60 * 60), // Convert interval to Unix timestamp
        nextReview.state, nextReview.lapses, locationId);
        // Record the review
