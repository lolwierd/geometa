...
async function fetchLearnableMetaData(panoId, mapId, source = "userscript") {
    ...
    try {
        ...
        logger.info(`📡 LearnableMeta API response: ${response.status} ${response.statusText}`);
        ...
        if (!response.ok) {
            throw new Error(`LearnableMeta API error: ${response.status} ${response.statusText}`);
        }
        const data = await response.json();
        logger.info(`✅ Successfully fetched meta data for ${data.country || "unknown country"}`);
        return data;
    }
    catch (error) {
...
router.post('/', async (req, res) => {
    try {
      ...
      RETURNING *
    `);
        const result = insertStmt.get(locationData);
        let message;
        if (result && result.id) {
